                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if "roles" in roles_data:
                        for role in roles_data["roles"]:
                            ...
                            available_roles.append(r_name)
                            # 使用大小寫不敏感或部分匹配來提高容錯率
                            for target in target_roles:
                                if target.lower() in r_name.lower():
                                    stats_dict[target] = role.get("memberCount", 0)
                    ...
                    if stats_lines:
                        ...
                        roles_str = ", ".join(available_roles) if available_roles else "無"
                        error_detail = roles_data.get("errors", [{"message": str(roles_data)}])[0].get("message", "未知")
                        reply = (f"【除錯模式】狀態碼: 200，但沒有匹配到階級。\n"
                                 f"實際抓到的階級名稱有: {roles_str}\n"
                                 f"原始資料: {error_detail}")
                        await message.reply(content=reply)
            except Exception as e:
