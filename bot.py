...
class MyClient(botpy.Client):
    async def on_group_at_message_create(self, message: Message):
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    group_res = await client.get(
                        f"https://groups.roproxy.com/v2/users/{user_id}/groups/roles",
                        timeout=10.0
                    )
                    ...
                    thumb_res = await client.get(
                        f"https://thumbnails.roproxy.com/v1/users/avatar-headshot?userIds={user_id}&size=420x420&format=Png",
                        timeout=10.0
                    )
        ...
        elif content.startswith("/groupstats"):
            args = content.replace("/groupstats", "").strip().upper()
            
            try:
                async with httpx.AsyncClient() as client:
                    roles_url = f"https://groups.roblox.com/v1/groups/{SCR_GROUP_ID}/roles"
                    roles_res = await client.get(roles_url, timeout=10.0)
                    
                    if roles_res.status_code != 200:
                        roles_url = f"https://groups.roproxy.com/v1/groups/{SCR_GROUP_ID}/roles"
                        roles_res = await client.get(roles_url, timeout=10.0)

                    roles_data = roles_res.json()
                    
                    target_roles_mapping = {
                        "[TD] Trainee Driver": "Trainee Driver",
                        "[QD] Qualified Driver": "Qualified Driver",
                        "[DS] Dispatcher": "Dispatcher",
                        "[GD] Guard": "Guard",
                        "[SG] Signaller": "Signaller",
                        "[SV] Supervisor": "Supervisor"
                    }
                    
                    arg_mapping = {
                        "TD": "[TD] Trainee Driver",
                        "QD": "[QD] Qualified Driver",
                        "DS": "[DS] Dispatcher",
                        "GD": "[GD] Guard",
                        "SG": "[SG] Signaller",
                        "SV": "[SV] Supervisor"
                    ...
                    if not args:
                        roles_to_find = target_roles_mapping.keys()
                        title_suffix = "TD-SV"
                    elif args in arg_mapping:
                        roles_to_find = [arg_mapping[args]]
                        title_suffix = args
                    else:
                        await message.reply(content="Error: 未知的階級代號，請使用 TD, QD, DS, GD, SG, SV。")
                        return
                    ...
                    if "roles" in roles_data:
                        for role in roles_data["roles"]:
                            r_name = role.get("name", "").strip()
                            if r_name in roles_to_find:
                                clean_name = target_roles_mapping[r_name]
                                stats_dict[clean_name] = role.get("memberCount", 0)
                    
                    stats_lines = []
                    for raw_name, clean_name in target_roles_mapping.items():
                        if raw_name in roles_to_find and clean_name in stats_dict:
                            stats_lines.append(f"{clean_name}: {stats_dict[clean_name]}")
                    
                    if stats_lines:
                        reply = (f"| SCR 階級人數統計 ({title_suffix})\n"
                                 f"----------------------------------\n"
                                 + "\n".join(stats_lines))
                        await message.reply(content=reply)
                    else:
                ...
                await message.reply(content=f"Error fetching groupstats: {str(e)}")
